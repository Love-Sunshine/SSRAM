...
class MGCA(nn.Module):
    def __init__(self, s_channels, c_channels):
        super().__init__()
        self.conv_1 = nn.Sequential(nn.BatchNorm2d(s_channels), nn.ReLU(), nn.Conv2d(s_channels, (s_channels + c_channels) // 2, 1, 1, 0))
        self.conv_2 = nn.Sequential(nn.BatchNorm2d(c_channels), nn.ReLU(), nn.Conv2d(c_channels, (s_channels + c_channels) // 2, 1, 1, 0))
        self.conv_3 = nn.Sequential(nn.BatchNorm2d((s_channels + c_channels) // 2), nn.ReLU(), nn.Conv2d((s_channels + c_channels) // 2, (s_channels + c_channels) // 2, 1, 1, 0))
        self.conv_4 = nn.Sequential(
            nn.BatchNorm2d((s_channels + c_channels) // 2 * 3),
            nn.ReLU(),
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d((s_channels + c_channels) // 2 * 3, (s_channels + c_channels) // 2, 1, 1, 0),
            nn.ReLU(),
            nn.Conv2d((s_channels + c_channels) // 2, (s_channels + c_channels) // 2, 1, 1, 0),
            nn.Sigmoid(),
        )

    def forward(self, x_s, x_c):
        attention = self.conv_4(torch.cat([self.conv_1(x_s), self.conv_2(x_c), self.conv_3(x_s + x_c)], dim=1))
        output = x_s * attention + x_c * (1 - attention)
        return output


class CG_CN(nn.Module):
    def __init__(self, config):
        ...
        self.mgca_1 = MGCA(decoder_channels[3], decoder_channels[3])
        self.mgca_2 = MGCA(decoder_channels[2], decoder_channels[2])
        self.mgca_3 = MGCA(decoder_channels[1], decoder_channels[1])
        self.mgca_4 = MGCA(decoder_channels[0], decoder_channels[0])
        self.sr_block_10 = SR_Block(decoder_channels[3], decoder_channels[3])
        self.sr_block_11 = SR_Block(decoder_channels[2], decoder_channels[2])
        self.sr_block_12 = SR_Block(decoder_channels[1], decoder_channels[1])
        self.sr_block_13 = SR_Block(decoder_channels[0], decoder_channels[0])
        self.pga_5 = PGA(decoder_channels[0], decoder_channels[0])
        self.mgca_5 = MGCA(decoder_channels[0], decoder_channels[0])
    ...
    def forward(self, x):
        ...
        feature_6 = self.sr_block_10(self.mgca_1(de_feature_3, self.conv_list[0](de_feature_4)))
        feature_7 = self.sr_block_11(self.mgca_2(de_feature_2, self.conv_list[1](feature_6)))
        feature_8 = self.sr_block_12(self.mgca_3(de_feature_1, self.conv_list[2](feature_7)))
        feature_9 = self.sr_block_13(self.mgca_4(feature_5, self.conv_list[3](feature_8)))
        output = self.output_layer(torch.cat([feature_5, feature_9, self.pga_5(feature_5, feature_9), self.mgca_5(feature_5, feature_9)], dim=1))
        return output
